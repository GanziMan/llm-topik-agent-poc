  ...
  .superRefine((data, ctx) => {
    if (data.problem_id === "51" || data.problem_id === "52") {
      const answerSchema = z.object({
        fill_blank_1: z.string(),
        fill_blank_2: z.string(),
      });
      ...
      if (!result.success) {
        ctx.addIssue({
          ...
          path: ["answer"],
          message:
            "For problem 51/52, answer must be an object with keys 'fill_blank_1' and 'fill_blank_2'",
        });
      }
    } else {
      // 53 or 54
    ...
    const agentInput = {
      ...
      answer_text:
        problem_id === "51" || problem_id === "52"
          ? JSON.stringify(answer) // 51/52는 {fill_blank_1, fill_blank_2} 객체를 문자열로
